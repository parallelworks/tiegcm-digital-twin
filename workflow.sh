...
if [[ ${input_method} == "TEXT" ]] || [[ ${input_method} == "WORKSPACE_PATH" ]]; then
    ...
    echo "WOrkSpace Script Path: ${workspace_script_path}"
    ...
    ssh ${resource_username}@${resource_publicIp} "chmod +x ${resource_script_path}"

cat <<EOF >job_metadata.sh
#!/bin/bash
export PW_JOB_ID=${PW_JOB_ID}
export PW_JOB_NUM=${PW_JOB_NUM}
export N_NODES=${nNodes}
export TILE_OUTPUTS=${tile_outputs}
EOF

    scp job_metadata.sh ${resource_username}@${resource_publicIp}:${resource_workdir}/job_metadata.sh
